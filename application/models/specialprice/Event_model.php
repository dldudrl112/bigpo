...
//        $this->load->library('session');
    ...
    public function eventListData($page = 1)
    {
        ...
        $sql="SELECT @rownum:=@rownum+1 num, idx, name,memo,hope,call2,note, reg_date from specialprice_event, (SELECT @rownum:= {$offset}) TMP ORDER BY idxs
    ...
    public function eventTotalNum() {
        ...
        $sql = "select count(*) count From specialprice_event ";
...
//        $idxs = $this->db->query($sql)->row()->idxs + 1;
        ...
        $sql="SELECT idxs FROM specialprice_event order by idxs DESC LIMIT 1";
        ...
        $result = $this->db->insert('specialprice_event', $insert_data);
        if($result) {
    ...
    public function eventData($idx) {
        $array = array($idx);
        $sql="SELECT *
            FROM specialprice_event WHERE idx = ?";
    ...
    public function eventEditData($update, $idx) {
        ...
        $result = $this->db->update('specialprice_event', $update);
    ...
    public function eventdel($idx) {
        $array = array($idx);
        $sql= "DELETE FROM specialprice_event WHERE idx = ?";
    ...
    public function eventOrder($idxs, $idx) {
        for($i=0; $i<count($idxs); $i++) {
            $sql= "UPDATE specialprice_event SET idxs = ? WHERE idx = ?";
